...
from chatbot_agent.policy.multiwoz_slot_trans import REF_USR_DA, REF_SYS_DA
...
class UserPolicyAgendaMultiWoz(Policy):
    ...
    def __init__(self, goal_model_path, db_path, vocab_path):
        """
        ...
        self.goal_generator = GoalGenerator(goal_model_path, db_path)

        self.__turn = 0
        self.goal = None
        self.agenda = None

        self.stand_value_dict = pnlp.read_json(os.path.join(vocab_path, "value_set.json"))
                    ...
                    for pairs in action[act]:
                        ...
                        if REF_SYS_DA_M[dom].get(pairs[0].lower(), None) is not None:
                            new_list.append([REF_SYS_DA_M[dom][pairs[0].lower()],
                                             self._normalize_value(dom, intent, REF_SYS_DA_M[dom][pairs[0].lower()], pairs[1])])

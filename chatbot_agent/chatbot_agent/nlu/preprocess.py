...
def preprocess(mode, data_dir, out_dir):
    ...
    for key in data_key:
        ...
        json.dump(processed_data[key], open(os.path.join(out_dir, '{}_data.json'.format(key)), 'w'), indent=2)

    print('dialog act num:', len(all_da))
    print('sentence label num:', len(all_intent))
    print('tag num:', len(all_tag))
    json.dump(all_da, open(os.path.join(out_dir, 'all_act.json'), 'w'), indent=2)
    json.dump(all_intent, open(os.path.join(out_dir, 'intent_vocab.json'), 'w'), indent=2)
    json.dump(all_tag, open(os.path.join(out_dir, 'tag_vocab.json'), 'w'), indent=2)

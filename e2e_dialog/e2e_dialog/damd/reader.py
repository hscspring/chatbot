...
from .clean_dataset import clean_text
from .ontology import all_domains, get_slot, dialog_acts, dialog_act_params, eos_tokens
from .utils import Vocab, padSeqs
...
class MultiWozReader(object):
    def __init__(self, vocab_path, data_path, db_processed_path):
        super().__init__()
        self.nlp = spacy.load('en_core_web_sm')
        self.db = MultiWozDB(db_processed_path, cfg.dbs)
        self.vocab_size = self._build_vocab(data_path)
        # self.domain_files = json.loads(open(cfg.domain_file_path, 'r').read())
        self.slot_value_set = json.loads(open(os.path.join(db_processed_path, cfg.slot_value_set_path), 'r').read())

        self.delex_sg_valdict_path = os.path.join(data_path, 'delex_single_valdict.json')
        self.delex_mt_valdict_path = os.path.join(data_path, 'delex_multi_valdict.json')
        self.ambiguous_val_path = os.path.join(data_path, 'ambiguous_values.json')
    ...
    def _build_vocab(self, data_path):
        ...
        self.vocab.load_vocab(os.path.join(data_path, cfg.vocab_path_eval))
        return self.vocab.vocab_size


    def delex_by_valdict(self, text, pair_file):
        text = clean_text(text, pair_file)
    ...
    def preprocess_utterance(self, user, pair_file):
        u = ' '.join(clean_text(user, pair_file).split())
        u_delex = self.delex_by_valdict(user, pair_file)
        return u, u_delex
